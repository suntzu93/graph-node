...
async fn test_abi_alloy_token_identity(api_version: Version) {
    ...
    let token_address = graph::abi::DecodedValue::Address(address.to_fixed_bytes().into());
    ...
    // Token::Bytes
    let token_bytes = graph::abi::DecodedValue::Bytes(vec![42, 45, 7, 245, 45]);
    ...
    let int_token = graph::abi::DecodedValue::Int(int, int.bits() as usize);
    ...
    let uint_token = graph::abi::DecodedValue::Uint(uint, uint.bit_len());
    ...
    // Token::Bool
    let token_bool = graph::abi::DecodedValue::Bool(true);
    ...
    // Token::String
    let token_string = graph::abi::DecodedValue::String("漢字Go🇧🇷".into());
    ...
    // Token::Array
    let token_array = graph::abi::DecodedValue::Array(vec![token_address, token_bytes, token_bool]);
    let token_array_nested = graph::abi::DecodedValue::Array(vec![token_string, token_array]);
    ...
    let new_token: graph::abi::DecodedValue = instance.asc_get(new_token_ptr).unwrap();
