...
impl HostExports {
    ...
    pub(crate) fn ethereum_decode(
        ...
        gas: &GasCounter,
        state: &mut BlockState,
    ) -> Result<graph::abi::DecodedValue, anyhow::Error> {
        Self::track_gas_and_ops(
            gas,
            state,
            gas::DEFAULT_GAS_OP.with_args(complexity::Size, &data),
        ...
        let param_types: graph::abi::ParamType = types.parse().context("Failed to read types")?;

        param_types.abi_decode(&data).context("Failed to decode")
    }
}
