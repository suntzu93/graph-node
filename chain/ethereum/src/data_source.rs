        ...
        match trigger {
            ...
            EthereumTrigger::Log(log_ref) => {
                ...
                let mut matching_handlers = valid_handlers
                    ...
                    .filter_map(|(event_handler, event_abi)| {
                        graph::abi::decode_log(event_abi, &log)
                            .map_err(|e| {
                                trace!(
                                    logger,
            ...
            EthereumTrigger::Call(call) => {
                ...
                let tokens = match function_abi
                    .abi_decode_input(&call.input.0[4..], true)
                    .with_context(|| {
                        format!(
                ...
                let inputs = tokens
                    .into_iter()
                    .enumerate()
                    .map(|(i, token)| graph::abi::DecodedParam {
                ...
                let tokens = function_abi
                    .abi_decode_output(&call.output.0, true)
                    .with_context(|| {
                        format!(
...
pub struct DeclaredCall {
    ...
    args: Vec<graph::abi::DecodedValue>,
}
...
pub struct MappingABI {
    pub name: String,
    pub contract: graph::abi::Contract,
}
...
impl CallDecl {
    fn address(&self, log: &Log, params: &[graph::abi::DecodedParam]) -> Result<H160, Error> {
    ...
    fn args(
        ...
        log: &Log,
        params: &[graph::abi::DecodedParam],
    ) -> Result<Vec<graph::abi::DecodedValue>, Error> {
        ...
        self.expr
            ...
            .iter()
            .map(|arg| match arg {
                CallArg::Address => Ok(DecodedValue::Address(log.address.to_fixed_bytes().into())),
                CallArg::HexAddress(address) => {
                    Ok(DecodedValue::Address(address.to_fixed_bytes().into()))
                }
                CallArg::Param(name) => {
                    let value = params
                        .iter()
