...
) -> Result<Option<Vec<graph::abi::DecodedValue>>, HostExportError> {
...
pub struct UnresolvedContractCall {
    ...
    pub function_args: Vec<graph::abi::DecodedValue>,
}
...
#[inline]
fn values_to_string(values: &[graph::abi::DecodedValue]) -> String {
    values
        .iter()
        .map(|x| x.to_string())
        .collect_vec()
