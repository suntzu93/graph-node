...
pub fn push_test_log(block: &mut BlockWithTriggers<Chain>, payload: impl Into<String>) {
    let log = Arc::new(Log {
        ...
        data: graph::abi::DecodedValue::String(payload.into())
            .abi_encode()
            .into(),
...
pub fn push_test_command(
    ...
    let log = Arc::new(Log {
        ...
        data: graph::abi::DecodedValue::Tuple(vec![
            graph::abi::DecodedValue::String(test_command.into()),
            graph::abi::DecodedValue::String(data.into()),
        ])
        .abi_encode_params()
        .into(),
        block_hash: Some(H256::from_slice(block.ptr().hash.as_slice())),
